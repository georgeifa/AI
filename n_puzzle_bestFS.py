...
def bestFS(initialState , finalState):
    ...
    return print("Found solution: ", current , " from the initial state of ",initialState," in ", moves, "moves")

def heuristic(states):
    scores = []
    for i in range(len(states)):
        scores.append(calculateManhattan(states[i]))

    sortedChildren = [x for _, x in sorted(zip(scores, states))]
...
def calculateManhattan(initial_state):
    ...
    for i,item in enumerate(initial_config):
        prev_row,prev_col = int(i/ dimmensions) , i % dimmensions
        goal_row,goal_col = int((item-1) /dimmensions),(item-1) % dimmensions
        if(item != 0):
...
bestFS([3,2,4,1,0,5,6,7,8],[1,2,3,4,5,6,7,8,0])
